...
import { useConnection, useReadContract, useWriteContract, useBalance } from "wagmi";
...
export function MiniPayDashboard() {
        ...
        if (celoBalance.value < MIN_BALANCE) {
          ...
          try {
            ...
            if (data.success) {
              toast({
                title: "🤖 Biota Agent",
                description: "Hemos fondeado tu MiniPay con gas (CELO) para que reclames tu UBI sin fricción.",
                duration: 6000
              });
              refetchBalance();
            }
  ...
  const activeAddress =
    viewAccount === "ubi" && identity.whitelistedRoot
      ? (identity.whitelistedRoot as `0x${string}`)
      : (primaryAddress as `0x${string}`);

  // 3. Multi-Token Balances
  const { balances, isLoading: loadingBalances } =
    useMultiTokenBalances(activeAddress);
  ...
  const handleClaim = () => {
    claimUbi(
      ...
      },
      {
        onSuccess: () => {
          alert(
            "¡UBI reclamado con éxito! Se sumará a tu saldo en unos segundos.",
          );
          refetchEntitlement();
        },
        onError: (err) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[80vh] p-8 text-center space-y-4">
        ...
        <p className="text-sm text-stone-400">
          Necesitamos tu wallet para empezar el goteo de G$.
        </p>
      </div>
    );
  }
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2">
            ...
            {primaryAddress && (
              <div 
                onClick={() => handleCopy(primaryAddress, 'MiniPay')}
                className="flex items-center gap-1 cursor-pointer hover:text-emerald-400 transition-colors bg-black/20 px-1.5 py-0.5 rounded-md border border-stone-800"
              >
                <span className="text-[8px] font-mono text-stone-400">
                  {primaryAddress.slice(0, 6)}...{primaryAddress.slice(-4)}
                </span>
                <Copy size={9} className="text-stone-500" />
              </div>
            )}
          </div>
          <Badge
            variant="outline"
            className="bg-stone-900 text-stone-300 border-stone-800 text-[8px] font-bold"
          >
            MINIPAY
          </Badge>
        </div>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="flex flex-col items-center border-r border-stone-800">
                <span className="text-[8px] font-black text-stone-500 uppercase tracking-widest mb-1">
                  CELO
                </span>
        ...
        <div className="flex items-center justify-between px-2">
          <div className="flex flex-col">
            ...
            {identity.whitelistedRoot && (
              <div 
                onClick={() => handleCopy(identity.whitelistedRoot as string, 'GoodWallet')}
              ...
              >
                <span className="text-[8px] text-blue-500 font-mono opacity-80">
                  {identity.whitelistedRoot.slice(0, 6)}...{identity.whitelistedRoot.slice(-4)}
                </span>
          ...
          <CardContent className="p-8 text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-center gap-1.5">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${stream.isActive ? "bg-emerald-500 animate-pulse" : "bg-stone-300"}`}
                />
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-[8px] text-stone-500 uppercase font-bold tracking-tighter">
                  {identity.hasValidIdentity
                    ? "Listo para recibir UBI"
                    : "Verifica tu rostro para empezar"}
                </p>
              </div>
              {!identity.hasValidIdentity && (
                <Button
