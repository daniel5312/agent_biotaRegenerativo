...
function LandingPage() {
  ...
  const handleCTA = (role: "PRODUCER" | "INVESTOR") => {
    if (typeof window !== "undefined") {
      localStorage.setItem("BIOTA_ROLE", role);
      window.dispatchEvent(new CustomEvent("biota-role-selected", { detail: role }));
    }
    if (!authenticated) {
      login();
    }
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (savedRole) {
      ...
      setActiveTab(savedRole === "PRODUCER" ? "pasaporte" : "mercado");
    }

    const handleRoleUpdate = (e: any) => {
      const role = e.detail;
      setUserRole(role);
      setActiveTab(role === "PRODUCER" ? "pasaporte" : "mercado");
    };
